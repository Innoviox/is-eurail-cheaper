...
let journey_search_url = "https://www.lefrecce.it/Channels.Website.BFF.WEB/website/ticket/solutions"
...
export default async function Trenitalia_price(legInfo) {
    return Promise.all([await station(legInfo.fromCity), await station(legInfo.toCity)])
        .then(cities => {
            return fetch(journey_search_url, {
                ...
                .then(result => {
                    ...
                    result.solutions.forEach(solution => {
                        ...
                        if (s.price !== null && s.trains.length === 1 && s.trains[0].description === legInfo.line.fahrtNr) {
