...
def extract_data(result) -> Journey:
    ...
    # hr, mn = preis.split("h")
    # todo interpret length
    return Journey(price=preis.text.strip(" €"), length=dauer.text)
...
class DBEngine(Engine):
    ...
    def get_journeys(self, from_city: str, to_city: str, date: dt.datetime) -> list[Journey]:
        ...
        von.click()
        slow_type(von, from_city)
        time.sleep(1)

        nach.click()
        slow_type(nach, to_city)
        ...
        loesungen = wait_for(driver, "reiseloesung-list-page__verbindung-list", by=By.CLASS_NAME,
                             criterion=EC.presence_of_element_located)
